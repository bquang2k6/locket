...
export const fetchUserPlan = async (userId) => {
  try {
    ...
    // Tìm thông tin chi tiết gói từ dữ liệu plans của chúng ta
    const planDetails = plans.find(p => p.id === planData.plan_id) || plans[0]; // Nếu không tìm thấy thì mặc định là gói free
